...
test("applyParameter() should replace matched %PARAMS%", () => {
    const res = applyParameter({ PASSWORD: "the secret is here" })(
...
test("applyParameter() should replace multiple params", () => {
    const res = applyParameter({ HOST: "localhost", PORT: "3346", X: "YZ" })(
...
test("applyParameter() should throw an error if param is missing, null or undefined", () => {
    expect(() =>
        applyParameter({ HoSt: "localhost", PORT: "3306" })(
            "mysql://user@pass:%HOST%:%PORT%",
        ),
    ).toThrow();
    expect(() =>
        applyParameter({ HOST: "localhost", PORT: null })(
            "mysql://user@pass:%HOST%:%PORT%",
        ),
    ).toThrow();
    expect(() =>
        applyParameter({ HOST: "localhost", PORT: undefined })(
            "mysql://user@pass:%HOST%:%PORT%",
        ),
    ).toThrow();
});
